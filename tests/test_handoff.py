...
class HandoffTests(unittest.TestCase):
    ...
    def test_gate_ids_and_statuses_are_unambiguous(self) -> None:
        ...
        self.assertTrue({item["status"] for item in gates}.issubset({"met", "unmet"}))
        g2 = next(item for item in gates if item["id"] == "G2")
        self.assertEqual(g2["status"], "met")
        self.assertEqual(
            g2["evidence"],
            "tests.test_readiness.ReadinessTests.test_sanitized_harbor_structural_fixture",
        )
    ...
    def test_human_router_names_machine_readable_authority(self) -> None:
        ...
        self.assertIn("Handoff readiness is not release authorization", router)
